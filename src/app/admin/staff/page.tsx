...
const StaffManagementPageComponent: React.FC = () => {
  ...
  const handleSaveStaff = async (staffData: StaffFormState, originalId?: string) => {
    ...
    try {
      if (originalId) { // Editing existing staff
        ...
        await apiClient.put(`/users/${originalId}`, staffToUpdate);
  ...
  const handleDeleteStaff = async (staff: User) => {
    ...
    setLoading(true);
    try {
      const result = await apiClient.delete(`/users/${staff.id}`);
  ...
  const tableHeaders: TableHeader<User>[] = [
    { 
      ...
      render: (item) => (
        <div className="flex items-center">
          {item.role === UserRole.ADMIN ? <ShieldCheckIcon className="h-5 w-5 mr-2 text-sky-600"/> : <UserIcon className="h-5 w-5 mr-2 text-gray-500"/>}
          {item.name}
        </div>
      )
    },
    { 
      ...
      label: UI_TEXT.ROLE,
      render: (item) => (
        <span className={`px-2 py-1 inline-flex text-sm leading-5 font-semibold rounded-full ${item.role === UserRole.ADMIN ? 'bg-sky-100 text-sky-800' : 'bg-green-100 text-green-800'}`}>
          {item.role}
        </span>
      )
    },
...
// 動的インポートでSSRを無効化
const StaffManagementPage = dynamic(() => Promise.resolve(StaffManagementPageComponent), {
  ssr: false,
});
