...
import { APP_TITLE, ADMIN_NAVIGATION_ITEMS, ROUTE_PATHS, UI_TEXT } from '../../constants';
import { ArrowLeftOnRectangleIcon, Bars3Icon, UserCircleIcon, ArrowUturnLeftIcon, XCircleIcon, ChevronDownIcon, BuildingStorefrontIcon } from '@heroicons/react/24/outline';

const AdminStoreSelector: React.FC = () => {
    const { stores, selectedStoreId, setSelectedStoreId, loading } = useStore();
    
    if (loading) {
        return <div className="text-sm text-gray-500">店舗読込中...</div>;
    }
    ...
    return (
        <div className="relative">
            <select
                ...
                aria-label="店舗を選択"
            >
                <option value="all">{UI_TEXT.ALL_STORES}</option>
                {stores.map(store => (
