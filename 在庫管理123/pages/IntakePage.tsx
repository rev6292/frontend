...
import { 
  ...
  RawInvoiceItem, ProcessedInvoiceItem, Category, PurchaseListItem as PLItemFromContext, PurchaseOrder, PurchaseOrderItem, PurchaseOrderStatus, ProductWithStock
} from '../types';
...
import { 
  ...
  DocumentTextIcon, EyeIcon, CheckIcon, QrCodeIcon, XMarkIcon
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input type="text" id="categoryName" value={name} onChange={e => setName(e.target.value)} required className="mt-1 block w-full p-2 border rounded-md bg-slate-900 text-white placeholder-slate-400 border-slate-700 focus:ring-indigo-500 focus:border-indigo-500" />
      </div>
      <div>
        <label htmlFor="parentCategory" className="block text-sm font-medium text-gray-700">親カテゴリ</label>
        <select id="parentCategory" value={parentId || ''} onChange={e => setParentId(e.target.value || null)} className="mt-1 block w-full p-2 border rounded-md bg-slate-900 text-white border-slate-700 focus:ring-indigo-500 focus:border-indigo-500">
...
const handleProcessedItemChange = (tempId: string, field: keyof ProcessedInvoiceItem, value: any) => {
    setParsedItems(prev => prev.map(item => {
        ...
        if (updatedItem.isNewProduct) {
          if (!(updatedItem.productName && updatedItem.barcode && updatedItem.categoryId && updatedItem.quantity >= 0 && updatedItem.pricePerUnit >= 0 && updatedItem.minimumStock >= 0)) {
...
const handleApplyBulkEdit = (field: keyof typeof bulkEditValues) => {
    const valueToApply = bulkEditValues[field];
    if (valueToApply === '' || selectedItemIds.size === 0) return;
    
    selectedItemIds.forEach(id => {
      handleProcessedItemChange(id, field as any, valueToApply);
    });
};
    ...
    setParsedItems(prev =>
      prev.map(item => {
        ...
        const updatedItem = { ...item, [field]: value };
         if (updatedItem.isNewProduct && updatedItem.productName && updatedItem.barcode && updatedItem.categoryId && updatedItem.pricePerUnit >= 0 && updatedItem.minimumStock >= 0) {
            updatedItem.status = 'ready';
        } else if (updatedItem.isNewProduct) {
            updatedItem.status = 'new_details_required';
        }
        return updatedItem;
      })
  ...
  const processedItemTableHeaders: TableHeader<ProcessedInvoiceItem>[] = [
    { key: 'selection', label: <input type="checkbox" checked={parsedItems.length > 0 && selectedItemIds.size === parsedItems.length} onChange={handleToggleSelectAll} className="form-checkbox h-4 w-4"/>, render: (item) => <input type="checkbox" checked={selectedItemIds.has(item._tempId)} onChange={() => handleToggleSelection(item._tempId)} className="form-checkbox h-4 w-4"/> },
    ...
    { key: 'productName', label: '商品名', render: (item) => item.isNewProduct ? <input type="text" value={item.productName} onChange={(e) => handleProcessedItemChange(item._tempId, 'productName', e.target.value)} className="w-full min-w-[200px] p-1 border rounded-md text-sm bg-slate-900 text-white border-slate-700"/> : <select value={item.matchedProductId || ''} onChange={(e) => handleProcessedItemChange(item._tempId, 'matchedProductId', e.target.value)} className="w-full min-w-[200px] p-1 border rounded-md text-sm bg-slate-900 text-white border-slate-700"><option value="">選択...</option>{products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select>},
    ...
    { key: 'categoryId', label: 'カテゴリ', render: (item) => {
        ...
        return (
            <div className="flex items-center gap-1">
                <select value={item.categoryId} onChange={(e) => handleProcessedItemChange(item._tempId, 'categoryId', e.target.value)} className="flex-grow w-full min-w-[200px] p-1 border rounded-md text-sm bg-slate-900 text-white border-slate-700">
    ...
    { key: 'minimumStock', label: '最低在庫', render: (item) => (item.isNewProduct ? <input type="number" value={item.minimumStock} onChange={(e) => handleItemChange(item._tempId, 'minimumStock', parseInt(e.target.value))} className="w-20 p-1 border rounded-md text-sm bg-slate-900 text-white border-slate-700" /> : <span>{products.find(p=>p.id === item.matchedProductId)?.minimumStock || '-'}</span>)},
      ...
      <details className="bg-white p-6 rounded-lg shadow-md group" open>
        ...
        <div className="mt-4 space-y-4">
            ...
            <div>
              ...
              <input ref={invoiceFileRef} type="file" id="invoice-image" accept="image/*" onChange={handleImageFileChange} className="block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100"/>
            </div>
        ...
        <div className="mt-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                  ...
                  <select id="supplier-select-barcode" value={selectedSupplierForBarcode} onChange={(e) => {setSelectedSupplierForBarcode(e.target.value); setIntakeCart([])}} className="w-full p-2 border rounded-md shadow-sm bg-slate-900 text-white border-slate-700 focus:ring-indigo-500 focus:border-indigo-500">
                      {suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                  </select>
              </div>
                              ...
                              {supplierCart.map(item => (
                                <li key={item.product.id} className="flex items-center justify-between py-3 text-sm">
                                  ...
                                  <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
                                    <input type="number" value={item.quantity || ''} onChange={e => updateQuantity(item.product.id, parseInt(e.target.value) || 0, date)} onFocus={e => e.target.select()} className="w-16 text-center border-gray-300 rounded-md py-1 bg-slate-100 text-gray-800"/>
                       ...
                       {po.items.map(item => (
                         <li key={item.productId} className={`flex flex-col md:flex-row gap-y-2 gap-x-4 items-start md:items-center py-4 px-2 ${item.isReceived ? 'opacity-60 bg-gray-100 rounded-md' : ''}`}>
                            <div className="w-full md:w-1/12 flex items-center gap-3">
                              <input type="checkbox" checked={receiptItems.get(item.productId)?.checked || false} onChange={e => handleReceiptItemChange(item.productId, 'checked', e.target.checked)} className="h-5 w-5 rounded form-checkbox text-indigo-600 focus:ring-indigo-500" disabled={item.isReceived} />
                        ...
                        {otherScheduledItems.map(item => (
                            <li key={item.id} className="p-3 flex flex-col sm:flex-row justify-between items-start sm:items-center">
                                ...
                                <div className="flex items-center gap-4 w-full sm:w-auto justify-between">
                                    ...
                                    <button onClick={() => { console.log("Edit item:", item); /* Future edit functionality */}} className="p-1 text-gray-400 hover:text-gray-600"><PencilIcon className="h-5 w-5"/></button>
                                </div>
                            </li>
                        ))}
                    </ul>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          <select value={filterYear} onChange={(e) => setFilterYear(e.target.value)} className="p-2 border rounded-md bg-slate-900 text-white border-slate-700">
            <option value="">全ての年</option>
            {years.map(y => <option key={y} value={y}>{y}年</option>)}
          </select>
          <select value={filterMonth} onChange={(e) => setFilterMonth(e.target.value)} className="p-2 border rounded-md bg-slate-900 text-white border-slate-700">
